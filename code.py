    ...
    def create(self, custom_X, custom_Y):
        ...
        try:
            if custom_X == None:
                ...
                xx = custom_X
            if custom_Y == None:
                yy = random.randint(100+self.size_y/2, height - 255)
            else:
                yy = custom_Y
...
def spawn_dogs(custom_budget = None, xxx = None, yyy = None):
    global dogs_spawning, health, dogs, cats, cat, dogs_variants, del_cat, money, catown, dogs_moneys, budget, frame_count, frame_count2, frame_count3, frame_count4, plustime, new_wey, wave, for_mega_wave, pause, music_playing, dog_damag_boost, cat_works_boost, cats_boost, upping, buy_cats, xexe, mx, dogs_class, scripts, mods_count_11d23s2saaa, catown2, pos, cursor_pos, bonus, new_music, event_game, maxMoney, input_text, running, input_active, ctrl_pressed, shift_pressed
    ...
    if online != 2:
        ...
        while remaining_budget > 0:
            dog_name = str(random.choice(list(dogs_spawning)))
...
def dogs_changed():
    global dogs_spawning, budget, dog_damag_boost, money, health, cat_works_boost, cats_boost, cats, dogs, bad_list, good_list, maxMoney, input_text, running, input_active, ctrl_pressed, shift_pressed
    global dogs_spawning, multipiller, unmultipiller, summa
    gebudged = budget
    dogs_spawning = []
    for i in range(5):
        gebudged += summa
        gebudged += round(gebudged/unmultipiller)
    for dog in dogs_variants:
        if dogs_variants[dog] < round(gebudged*multipiller):
            dogs_spawning.append(dog)
    return gebudged
...
def start(online):
    loading_text('игры')
    global final_wave, end_boss, boss_steps, health, Number, TimeStart, event_game, dogs, cats, cat, dogs_variants, del_cat, money, catown, dogs_moneys, budget, frame_count, frame_count2, frame_count3, frame_count4, plustime, new_wey, wave, for_mega_wave, pause, music_playing, dog_damag_boost, cat_works_boost, cats_boost, upping, buy_cats, xexe, mx, dogs_class, __dangerous_keywords_pp2irooodjhjjjkjkn, scripts, bad_list, good_list, catown2, bonus, new_music, buy_no_cats, event_game, maxMoney, cat_unblock, cats_moneys, cats_dogs_spawns
    global dogs_spawning, multipiller, unmultipiller, summa
    for script in scripts:
    ...
    for dog in dogs_variants:
        if dogs_variants[dog]/50 < budget:
            dogs_spawning.append(dog)
...
def game(online):
    global final_wave, end_boss, boss_steps, health, TimeStart, Number, dogs, cats, cat, dogs_variants, del_cat, money, catown, dogs_moneys, budget, frame_count, frame_count2, frame_count3, frame_count4, plustime, new_wey, wave, for_mega_wave, pause, music_playing, dog_damag_boost, cat_works_boost, cats_boost, upping, buy_cats, xexe, mx, dogs_class, scripts, mods_count_11d23s2saaa, catown2, pos, cursor_pos, bonus, new_music, event_game, maxMoney, input_text, running, input_active, ctrl_pressed, shift_pressed, stop_game, pos
    running = True
    bonus = False
    clock = pygame.time.Clock()
    cursor_pos = (0,0)
